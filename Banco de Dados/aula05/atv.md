## Parte 01 Rechonhecimento De Dados
...
```sql
SELECT nome,valor FROM livro ORDER BY valor DESC LIMIT 5;
```
![alt text](image-6.png)

 Listando os 5 livros com menor estoque (título e estoque):
 ```sql
 SELECT nome FROM livro ORDER BY estoque  LIMIT 5;
 ```
 ![alt text](image-7.png)

 ## Parte 02 Filtros Numéricos

 Mostrando titulo e estoque de todos os livros do gênero Técnico:
 ```sql
 SELECT nome,estoque FROM livro WHERE genero LIKE 'Técnico%';
 ```
 ![alt text](image-8.png)

Mostrando titulo e preco dos livros que custam mais de R$ 200,00:
```sql
SELECT nome,valor FROM livro WHERE valor >= 200;
```
![alt text](image-9.png)

 Mostrando titulo e preco dos livros com preço entre R$ 40,00 e R$ 70,00:
 ```sql
 SELECT nome,valor FROM livro WHERE valor BETWEEN 40 and 70;
 ```
 ![alt text](image-10.png)

 Mostrando os livros com estoque abaixo de 5 unidades (situação de reposição urgente):
 ```sql
 SELECT nome,estoque FROM livro ORDER BY estoque LIMIT 5;
 ```
 ![alt text](image-11.png)

 Listando os livros publicados antes de 1900, ordenados do mais antigo para o mais recente:
 ```sql
  SELECT nome,ano_publicacao FROM livro WHERE ano_publicacao > 1900;
  ```
  ![alt text](image-12.png)

 Listando os livros publicados entre 2010 e 2020, mostrando título, ano e gênero:
```sql
 SELECT nome,genero,ano_publicacao FROM livro WHERE ano_publicacao BETWEEN 2010 AND 2020;
```
